...
import collections
import os
from workload_generator.constants import DIRECTORY_DEPTH_PROBABILITY, FS_SNAPSHOT_PATH
...
class DirectoryTreeManager(object):
    ...
    def initialize_file_system_tree(self, fs_snapshot_path):
        for top, dirs, files in os.walk(fs_snapshot_path):
            ...
            for directory in dirs:
                add_fs_node(self.file_system, top+directory)
            for the_file in files:
                add_fs_node(self.file_system, top+the_file)
    ...
    def get_fitness_proportionate_element(self, probabilities_dict):
        file_type = None
        random_trial = self.random_generator.random()
        start_range = 0.0
    ...
    def get_file_based_on_type_popularity(self, stereotype_file_types_probabilities, file_type_extensions):
        ...
        while len(tested_file_types) < len(stereotype_file_types_probabilities):
            ...
            file_type = self.get_fitness_proportionate_element(stereotype_file_types_probabilities)
            if file_type in tested_file_types: continue
            all_files_of_type = self.get_fs_files_of_type(file_type_extensions[file_type], FS_SNAPSHOT_PATH)
            tested_file_types.add(file_type)
            if all_files_of_type != []:
                selected_file =  self.random_generator.choice(all_files_of_type)
                break
        return selected_file, file_type
    
    '''Get a random directory from the whole tree'''
    def get_random_fs_directory(self, base_path=''):
        '''Get only directories from this tree level'''
        fs_level_directories = [fs_node for fs_node in get_tree_node(self.file_system, base_path) if '.' not in fs_node]
        base_path += os.sep
        ...
        fs_node = self.random_generator.choice(fs_level_directories)
        '''If the trial succeeds, this will be the new location'''
        if self.random_generator.random() < DIRECTORY_DEPTH_PROBABILITY:
            return base_path + fs_node + os.sep
        '''If not, continue the random navigation'''
        return self.get_random_fs_directory(base_path + fs_node)
    
    def get_empty_directory(self, base_path=''):
        '''Get only directories from this tree level'''
        this_level_nodes = get_tree_node(self.file_system, base_path)
        ...
        self.random_generator.shuffle(fs_level_directories)
        '''If we can choose among several directories, pick one'''
        for fs_dir in fs_level_directories:
            '''If the trial succeeds, this will be the new location'''
            empty_dir = self.get_empty_directory(base_path + fs_dir)
            if empty_dir != None: return empty_dir
        '''If not, continue the random navigation'''
        return None
    
    def get_fs_files_of_type(self, file_type_extensions, base_path=''):
        ...
        fs_level_files = [base_path+fs_node for fs_node in this_level_nodes if '.' in fs_node and \
                          self.matches_file_type(fs_node, file_type_extensions)]
        ...
        for fs_dir in fs_level_directories:
            ...
            fs_level_files += self.get_fs_files_of_type(file_type_extensions, base_path + fs_dir)
        '''Return the aggregated list'''
        return fs_level_files
    
    def matches_file_type(self, fs_node, file_type_extensions):
        for extension in file_type_extensions: 
            if extension in fs_node: return True
        return False
    
    def get_type_of_file(self, fs_node, file_type_extensions):
    ...
    def add_node_to_fs(self, created_file):
        add_fs_node(self.file_system, created_file)
        
    def delete_node_from_fs(self, deleted_file):
        delete_fs_node(self.file_system, deleted_file)
