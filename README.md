...
TODO: Chenglong.
Prerequisites:
currently only tested with linux-mint 16 and debian 7

## Getting started:

BenchBox Deployment has Master Slave architecture where the master node is defined with the role as manager, from the manager we can send remote procedure calls by SSH to the slave machines.
